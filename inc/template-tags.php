...
function altertech_s_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && altertech_s_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( '<i class="mdi-action-group-work"></i> <strong>Category:</strong> %1$s', 'altertech_s' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'altertech_s' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( '<i class="mdi-action-loyalty"></i> <strong>Tag:</strong>  %1$s', 'altertech_s' ) . '</span></br>', $tags_list );
		}
	}
	...
	edit_post_link( __( 'Edit Post', 'altertech_s' ), '<span class="edit-link button--secondary"><i class="mdi-content-create"></i>', '</span>' );
}
endif;
